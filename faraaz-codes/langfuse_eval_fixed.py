        ...
        for case_idx, case in enumerate(cases):
            ...
            with langfuse.start_as_current_observation(
                ...
                with propagate_attributes(
                    ...
                    metadata={"eval_dataset": dataset_name, "eval_case": str(case_idx)},
                ):
                    try:
                ...
                for turn_idx, (turn, response) in enumerate(
                    ...
                    await asyncio.sleep(0.3)  # stay inside rate limits
                ...
                helpfulness_str = f"{avg_helpfulness:.2f}" if avg_helpfulness is not None else "N/A"

                print(
                    f"  ✅ tool_acc={tool_score:.2f} | "
                    f"helpfulness={helpfulness_str} | "
...
def print_summary(results: list[dict]) -> None:
    ...
    for r in results:
        helpful = f"{r['avg_helpfulness']:.2f}" if r["avg_helpfulness"] is not None else "  N/A"
        print(
            f"  {r['case']:<30} "
    ...
    h_vals   = [r["avg_helpfulness"] for r in results if r["avg_helpfulness"] is not None]
